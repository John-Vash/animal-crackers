...
public class PlaneFittingActivity extends Activity implements View.OnTouchListener {
        ...
        synchronized (this) {
            if (mIsConnected) {
                ...
                try {
                    ...
                    Log.e(TAG, "Tango error while disconnecting", e);
                }
                mIsConnected = false;
            }
        }
    ...
    private void connectTango() {
        ...
        try {
            ...
            Log.e(TAG, "Need ADF Permissions", se);
        }
        ...
        mTango.connectListener(framePairs, new OnTangoUpdateListener() {
            ...
            public void onPoseAvailable(TangoPoseData pose) {
                ...
                final double adDeltaTime = (pose.timestamp - mPreviousPoseTimeStamp) *
                        SECS_TO_MILLISECS;
                mPreviousPoseTimeStamp = pose.timestamp;
                mTimeToNextUpdate -= adDeltaTime;

                if (mTimeToNextUpdate < 0.0) {
                    mTimeToNextUpdate = UPDATE_INTERVAL_MS;

                    //print localized status to LOG
                    Log.d(TAG, "Localized: " + mIsRelocalized);
                }
            }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        //requestCode == REQUEST_CODE_TANGO_PERMISSION
        if (requestCode == Tango.TANGO_INTENT_ACTIVITYCODE) {
            // Make sure the request was successful
            if (resultCode == RESULT_CANCELED) {
                Toast.makeText(this, "Area Description Permission Needed!", Toast.LENGTH_SHORT).show();
                finish();
            }
        }
    }
